...
func update_selected_block(index: int):
	...
	hotbar_selection.offset_left = selected_offset - HOTBAR_SELECTION_TEXTURE_SIZE * ICON_SCALE / 2.0
	hotbar_selection.offset_right = selected_offset + HOTBAR_SELECTION_TEXTURE_SIZE * ICON_SCALE / 2.0
...
func set_hotbar_preview(face_kind: VoxelWorld.FaceKind, index: int):
	...
	$FixedHUD/hotbarPreviewcontainer.add_child(rect)
	...
	var preview_offset = get_element_offset(index)
	rect.anchor_left = 0.5
	rect.anchor_right = 0.5
	rect.anchor_top = 0.898
	rect.anchor_bottom = 0.898
	var preview_size = preview_viewport.size.x
	rect.offset_left = preview_offset - preview_size / 2.0
	rect.offset_right = preview_offset + preview_size / 2.0
...
func __update_preview_rect_texture(rect: TextureRect, viewport: SubViewport):
	# Skip two frames to guarantee that the texture will be available
